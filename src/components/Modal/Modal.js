...
const renderMenuTypes = ({
  ...
  const url = `${SERVER_ROUTE}/api/account/menuType`;
  return (
    <>
      {destiny === "edit" ? (
        <EditModal
...
const renderMenuItems = ({
  ...
  const url = `${SERVER_ROUTE}/api/account/menuItem`;
  ...
  const handleImageChange = async (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const result = await axios.post(
        `${SERVER_ROUTE}/api/account/image-upload`,
        formData,
        {
          headers: {
      ...
      if (result && result.data && result.data.url) {
        setImage(`${SERVER_ROUTE}/${result.data.url}`);
      }
    }
  };
